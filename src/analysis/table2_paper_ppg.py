...
Generates the formatted "Table" (mean ± std per activity) from the
per-record results of benchmark_paper_strict_ppg.py.

Run AFTER benchmark_paper_strict_ppg.py has produced benchmark_results_paper_strict_ppg.csv.
"""
...
INPUT_CSV = '../../results/benchmark_results_paper_strict_ppg.csv'
OUTPUT_CSV = '../../results/table2_paper_ppg.csv'
...
# Display
print("\n" + "=" * 100)
print("  TABLE — PAPER STRICT REPRODUCTION PPG (mean ± std per activity)")
print("=" * 100)
...
summary_fmt.to_csv(OUTPUT_CSV.replace('.csv', '_formatted2_ppg.csv'))
print(f"Wrote: {OUTPUT_CSV} (raw mean/std)")
print(f"Wrote: {OUTPUT_CSV.replace('.csv', '_formatted2_ppg.csv')} (mean ± std strings)\n")
